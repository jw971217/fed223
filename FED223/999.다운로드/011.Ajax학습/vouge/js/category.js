...
$(()=>{
    ...
    // -> 미리 세팅된 배경이미지가 컨텐츠 박스에 나옴
    $('.cont').addClass(data['경로']);

    // (3) LNB메뉴 세팅하기
    // "메뉴" 데이터 값으로 "없음"이 아닐경우 
    // 메뉴수 만큼 ul>li>a의 구조로 메뉴를 구성함
    let menu = data['메뉴']
    console.log('메뉴', menu)
...
$(()=>{////////////// jQB ////////////////////////
    ...
    /*************************************************************** 
        카테고리 페이지 타이틀 한글자씩 두두등장 애니
    ***************************************************************/
    
    // 대상: .stit 
    // 방법: 서브타이틀을 가져와서 각각 span 태그로 싸준다!
    const stit = $(".stit");

    // 1. 서브타이틀 글자 읽어오기
    let stxt = $('.stit').text();
    console.log('이거야',stxt);
    ...
    // 6. LNB메뉴가 있으면 조금 있다가 나와라!
    $(".lnb").fadeTo(1,0).delay(500).fadeTo(500,1);
    // fadtTo(시간, 오파, 이징, 함수)
    // -> fadeIn/fadiOut과 달리 display를 none하지 않음!
